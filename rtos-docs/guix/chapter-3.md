...
ほとんどの開発ツールでは、アプリケーション プログラム イメージを "*命令*"、"*定数*"、"*初期化されたデータ*"、"*初期化されていないデータ*"、"*GUIX スレッド スタック*" の 5 つの基本的な領域に分割します。 X ページの図 X は、これらのメモリ領域の例を示しています。
...
ランタイム メモリ割り当ておよび割り当て解除サービスを必要とする GUIX サービスには、次のものが含まれます。
  ...
  - ソフトウェア ランタイム jpeg イメージ デコーダー。

  - ソフトウェア ランタイム png イメージ デコーダー。

  - GX_STYLE_TEXT_COPY でのテキスト ウィジェットの使用。

  - ランタイム ピクセルマップ サイズ変更および回転ユーティリティ関数。
