...
組み込みネットワーク アプリケーションは、携帯電話、通信機器、自動車エンジン、レーザー プリンター、医療機器などの製品内部のマイクロプロセッサで実行する、ネットワークへのアクセスが必要なアプリケーションです。 これらのアプリケーションはほとんどの場合メモリとパフォーマンスの制約があります。 また、そのソフトウェアとハードウェアに専用の用途があることも、埋め込みネットワーク アプリケーションの特徴の 1 つです。
