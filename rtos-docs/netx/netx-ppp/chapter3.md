...
この章では、すべての Azure RTOS NetX PPP サービス (以下に記載) をアルファベット順に説明します。
...
- **nx_ppp_dns_address_get**: "*DNS の IP アドレスを取得する*"
...
このサービスを使用すると、指定した NetX IP インスタンスの PPP インスタンスが作成されます。
...
このサービスを使用すると、ピアによって提供された DNS IP アドレスが取得されます。 ピアから IP アドレスが提供されていない場合は、IP アドレスとして 0 が返されます。

### <a name="input-parameters"></a>入力パラメーター

- **ppp_ptr**: PPP 制御ブロックへのポインター。
- **dns_address_ptr**: DNS IP アドレスの格納先。

### <a name="return-values"></a>戻り値

- **NX_SUCCESS**: (0x00) PPP アドレスの取得に成功しました。
...
このサービスを使用すると、インターネット プロトコル制御プロトコル (IPCP) で使用するローカルおよびピアの IP アドレスが設定されます。 PPP アプリケーションでは、それ自体と他のピアで有効な IP アドレスを持つ PPP インスタンスで、このサービスを呼び出す必要があります。  有効なアドレスが PPP インスタンスに登録されていない場合は、PPP ピアに依存してその IP アドレスを定義する必要があります。
...
このサービスを使用すると、LCP ping 要求が送信され、PPP デバイスがエコー応答を待機していることを示すフラグが設定されます。 要求が送信されるとすぐに、サービスに制御が戻ります。 応答を待機することはありません。 
...
このサービスを使用すると、PPP 以外の ASCII 文字列が PPP インターフェイスから直接送信されます。 通常、モデム制御情報を含む PPP 以外のパケットを PPP が受信した後に使用されます。
