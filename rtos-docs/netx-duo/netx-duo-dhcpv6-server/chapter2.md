...
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a>第 2 章 - Azure RTOS NetX Duo DHCPv6 サーバーのインストールと使用

この章では、NetX Duo DHCPv6 サーバーのインストール、セットアップ、使用に関連するさまざまな問題について説明します。
...
**nxd_dhcpv6_server**: NetX DuoDHCPv6Server ヘッダー ファイル

**nxd_dhcpv6_server.c**: NetX DuoDHCPv6Server ソース ファイル

**demo_netxduo_dhcpv6.c**: NetX Duo DHCPv6 Server デモ ファイル

**nxd_dhcpv6_server.pdf**: NetX Duo DHCPv6Server ユーザー ガイド

## <a name="netx-duo-dhcpv6-server-installation"></a>NetX Duo DHCPv6 サーバーのインストール

NetX Duo DHCPv6Server API を使用するには、前述のディストリビューション全体を、NetX Duo がインストールされているのと同じディレクトリにコピーする必要があります。 たとえば、NetX Duo が " *\threadx\arm7\green*" ディレクトリにインストールされている場合は、*nxd_dhcpv6_server.h* ファイルと *nx_dhpcv6_server.c* ファイルをこのディレクトリにコピーする必要があります。

## <a name="using-netx-duo-dhcpv6-server"></a>NetX Duo DHCPv6 サーバーの使用

NetX Duo DHCPv6Server API の使用は簡単です。 基本的には、アプリケーション コードに ThreadX と NetX を使用するための *tx_api.h* と *nx_api.h* をインクルードしてから *nx_dhcpv6-server.h* をインクルードする必要があります。 さらに、アプリケーションのビルド プロセス中に *nxd_dhcpv6_server.c* をインクルードする必要があります。 このファイルは、他のアプリケーション ファイルと同様にコンパイルする必要があり、そのオブジェクト形式は、アプリケーションのファイルと一緒にリンクする必要があります。 NetX Duo DHCPv6 サーバーを使用するために必要なことはこれだけです。

DHCPv6is は IPv6 プロトコルに基づいているため、*nxd_ipv6_enable* を使用して、IP インスタンスで IPv6 を有効にする必要があることに注意してください。 NetX Duo UDP サービスと ICMPv6 サービスも利用されています。 UDP は、NetX Duo DHCPv6 サーバー スレッド タスクを開始する前の *nxd_icmp_enable* の呼び出しによって有効になる *nx_udp_enable* と ICMPv6is を呼び出すことによって有効になります。

## <a name="small-example-system"></a>小規模のシステム例

NetX Duo DHCPv6 サーバーを簡単に使用する例について、仮想 "RAM" ドライバーを介して実行される DHCPv6 クライアントとサーバーを使用する次の小さな例で説明します。 このデモでは、NetX Duo 環境を使用する単一のホーム ホストを想定しています。

*tx_application_define* によって、クライアントとサーバーの両方に対して、DHCPv6 メッセージを送信するためのパケット プール、スレッド、および IP インスタンスが作成され、行 116 から 157 で、クライアントとサーバーの両方に対して、UDP (UDP で実行される DHCP)、IPv6、ICMP、および ICMPv6 が有効になります。

DHCPv6 サーバーは、行 456 で作成されます。 オプションのアドレス拒否またはオプションの要求ハンドラーは定義されません。 行 435 から 行 453 のサーバー スレッド エントリ関数で、サーバー IP とリンク ローカル アドレス サービスが設定されます。

DHCPv6 サーバーを起動する前に、ホスト アプリケーションによって、行 498 でサーバー DUID が作成され、行 483 でローカル ネットワーク DNS サーバーが設定されます。 次に、行 521 で、割り当て可能な IP アドレスのテーブルが作成されます。 サーバー テーブルをメモリに格納および取得する方法については、付録 D の「**高度なシステム例**」を参照してください。

その後、行 530 で、DHCPv6 サーバーを起動する準備が整います。

NetX Duo DHCPv6 クライアントの作成と実行の詳細については、DHCPv6 サーバーと共に配布される *nxd_dhcpv6_client.pdf* ファイルを参照してください。
