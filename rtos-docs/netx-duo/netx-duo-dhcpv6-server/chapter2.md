---
title: 第 2 章 - Azure RTOS NetX Duo DHCPv6 サーバーのインストールと使用
description: この章では、NetX Duo DHCPv6 サーバーのインストール、セットアップ、使用に関連するさまざまな問題について説明します。
author: philmea
ms.author: philmea
ms.date: 06/08/2020
ms.topic: article
...
ms.locfileid: "104810853"
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a><span data-ttu-id="eb996-103">第 2 章 - Azure RTOS NetX Duo DHCPv6 サーバーのインストールと使用</span><span class="sxs-lookup"><span data-stu-id="eb996-103">Chapter 2 - Installation and use of Azure RTOS NetX Duo DHCPv6 server</span></span>

<span data-ttu-id="eb996-104">この章では、NetX Duo DHCPv6 サーバーのインストール、セットアップ、使用に関連するさまざまな問題について説明します。</span><span class="sxs-lookup"><span data-stu-id="eb996-104">This chapter contains a description of various issues related to installation, setup, and usage of the NetX Duo DHCPv6 Server.</span></span>

## <a name="product-distribution"></a><span data-ttu-id="eb996-105">製品の配布</span><span class="sxs-lookup"><span data-stu-id="eb996-105">Product Distribution</span></span>

<span data-ttu-id="eb996-106">Azure RTOS NetX Duo は、[https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/) のパブリック ソース コード リポジトリから入手できます。</span><span class="sxs-lookup"><span data-stu-id="eb996-106">Azure RTOS NetX Duo can be obtained from our public source code repository at [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/).</span></span>

<span data-ttu-id="eb996-107">**nxd_dhcpv6_server**: NetX DuoDHCPv6Server ヘッダー ファイル</span><span class="sxs-lookup"><span data-stu-id="eb996-107">**nxd_dhcpv6_server.h** NetX DuoDHCPv6Server header file</span></span>

<span data-ttu-id="eb996-108">**nxd_dhcpv6_server.c**: NetX DuoDHCPv6Server ソース ファイル</span><span class="sxs-lookup"><span data-stu-id="eb996-108">**nxd_dhcpv6_server.c** NetX DuoDHCPv6Server source file</span></span>

<span data-ttu-id="eb996-109">**demo_netxduo_dhcpv6.c**: NetX Duo DHCPv6 Server デモ ファイル</span><span class="sxs-lookup"><span data-stu-id="eb996-109">**demo_netxduo_dhcpv6.c** NetX Duo DHCPv6 Server demo file</span></span>

<span data-ttu-id="eb996-110">**nxd_dhcpv6_server.pdf**: NetX Duo DHCPv6Server ユーザー ガイド</span><span class="sxs-lookup"><span data-stu-id="eb996-110">**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server User Guide</span></span>

## <a name="netx-duo-dhcpv6-server-installation"></a><span data-ttu-id="eb996-111">NetX Duo DHCPv6 サーバーのインストール</span><span class="sxs-lookup"><span data-stu-id="eb996-111">NetX Duo DHCPv6 Server Installation</span></span>

<span data-ttu-id="eb996-112">NetX Duo DHCPv6Server API を使用するには、前述のディストリビューション全体を、NetX Duo がインストールされているのと同じディレクトリにコピーする必要があります。</span><span class="sxs-lookup"><span data-stu-id="eb996-112">In order to use the NetX Duo DHCPv6Server API, the entire distribution mentioned previously should be copied to the same directory where NetX Duo is installed.</span></span> <span data-ttu-id="eb996-113">たとえば、NetX Duo が " *\threadx\arm7\green*" ディレクトリにインストールされている場合は、*nxd_dhcpv6_server.h* ファイルと *nx_dhpcv6_server.c* ファイルをこのディレクトリにコピーする必要があります。</span><span class="sxs-lookup"><span data-stu-id="eb996-113">For example, if NetX Duo is installed in the directory “*\threadx\arm7\green*” then the *nxd_dhcpv6_server.h* and *nx_dhpcv6_server.c* files should be copied into this directory.</span></span>

## <a name="using-netx-duo-dhcpv6-server"></a><span data-ttu-id="eb996-114">NetX Duo DHCPv6 サーバーの使用</span><span class="sxs-lookup"><span data-stu-id="eb996-114">Using NetX Duo DHCPv6 Server</span></span>

<span data-ttu-id="eb996-115">NetX Duo DHCPv6Server API の使用は簡単です。</span><span class="sxs-lookup"><span data-stu-id="eb996-115">Using the NetX Duo DHCPv6Server API is easy.</span></span> <span data-ttu-id="eb996-116">基本的には、アプリケーション コードに ThreadX と NetX を使用するための *tx_api.h* と *nx_api.h* をインクルードしてから *nx_dhcpv6-server.h* をインクルードする必要があります。</span><span class="sxs-lookup"><span data-stu-id="eb996-116">Basically, the application code must include *nx_dhcpv6-server.h* after it includes *tx_api.h* and *nx_api.h*, in order to use ThreadX and NetX Duo, respectively.</span></span> <span data-ttu-id="eb996-117">さらに、アプリケーションのビルド プロセス中に *nxd_dhcpv6_server.c* をインクルードする必要があります。</span><span class="sxs-lookup"><span data-stu-id="eb996-117">The application must also include *nxd_dhcpv6_server.c* in the build process.</span></span> <span data-ttu-id="eb996-118">このファイルは、他のアプリケーション ファイルと同様にコンパイルする必要があり、そのオブジェクト形式は、アプリケーションのファイルと一緒にリンクする必要があります。</span><span class="sxs-lookup"><span data-stu-id="eb996-118">This file must be compiled in the same manner as other application files and its object form must be linked along with the files of the application.</span></span> <span data-ttu-id="eb996-119">NetX Duo DHCPv6 サーバーを使用するために必要なことはこれだけです。</span><span class="sxs-lookup"><span data-stu-id="eb996-119">This is all that is required to use NetX Duo DHCPv6 Server.</span></span>

<span data-ttu-id="eb996-120">DHCPv6is は IPv6 プロトコルに基づいているため、*nxd_ipv6_enable* を使用して、IP インスタンスで IPv6 を有効にする必要があることに注意してください。</span><span class="sxs-lookup"><span data-stu-id="eb996-120">Note that since DHCPv6is based on the IPv6 protocol, IPv6 must be enabled on the IP instance using *nxd_ipv6_enable*.</span></span> <span data-ttu-id="eb996-121">NetX Duo UDP サービスと ICMPv6 サービスも利用されています。</span><span class="sxs-lookup"><span data-stu-id="eb996-121">NetX Duo UDP and ICMPv6 services are also utilized.</span></span> <span data-ttu-id="eb996-122">UDP は、NetX Duo DHCPv6 サーバー スレッド タスクを開始する前の *nxd_icmp_enable* の呼び出しによって有効になる *nx_udp_enable* と ICMPv6is を呼び出すことによって有効になります。</span><span class="sxs-lookup"><span data-stu-id="eb996-122">UDP is enabled by calling *nx_udp_enable* and ICMPv6is enabled by calling *nxd_icmp_enable* prior to starting the NetX Duo DHCPv6 Server thread task.</span></span>

## <a name="small-example-system"></a><span data-ttu-id="eb996-123">小規模のシステム例</span><span class="sxs-lookup"><span data-stu-id="eb996-123">Small Example System</span></span>

<span data-ttu-id="eb996-124">NetX Duo DHCPv6 サーバーを簡単に使用する例について、仮想 "RAM" ドライバーを介して実行される DHCPv6 クライアントとサーバーを使用する次の小さな例で説明します。</span><span class="sxs-lookup"><span data-stu-id="eb996-124">An example of how easy it is to use the NetX Duo DHCPv6 Server is described in the small example below using a DHCPv6 Client and Server running over a virtual “RAM” driver.</span></span> <span data-ttu-id="eb996-125">このデモでは、NetX Duo 環境を使用する単一のホーム ホストを想定しています。</span><span class="sxs-lookup"><span data-stu-id="eb996-125">This demo assumes a single homed host using the NetX Duo environment.</span></span>

<span data-ttu-id="eb996-126">*tx_application_define* によって、クライアントとサーバーの両方に対して、DHCPv6 メッセージを送信するためのパケット プール、スレッド、および IP インスタンスが作成され、行 116 から 157 で、クライアントとサーバーの両方に対して、UDP (UDP で実行される DHCP)、IPv6、ICMP、および ICMPv6 が有効になります。</span><span class="sxs-lookup"><span data-stu-id="eb996-126">*tx_application_define* creates packet pool for sending DHCPv6 message, a thread and an IP instance for both the Client and Server, and enables UDP (DHCP runs over UDP), IPv6, ICMP and ICMPv6 for both Client and Server IP tasks in lines 116-157.</span></span>

<span data-ttu-id="eb996-127">DHCPv6 サーバーは、行 456 で作成されます。</span><span class="sxs-lookup"><span data-stu-id="eb996-127">The DHCPv6 Server is created in line 456.</span></span> <span data-ttu-id="eb996-128">オプションのアドレス拒否またはオプションの要求ハンドラーは定義されません。</span><span class="sxs-lookup"><span data-stu-id="eb996-128">It does not define the optional address decline or option request handlers.</span></span> <span data-ttu-id="eb996-129">行 435 から 行 453 のサーバー スレッド エントリ関数で、サーバー IP とリンク ローカル アドレス サービスが設定されます。</span><span class="sxs-lookup"><span data-stu-id="eb996-129">In the Server thread entry function, the Server IP is set up with a link local address services in lines 435-453.</span></span>

<span data-ttu-id="eb996-130">DHCPv6 サーバーを起動する前に、ホスト アプリケーションによって、行 498 でサーバー DUID が作成され、行 483 でローカル ネットワーク DNS サーバーが設定されます。</span><span class="sxs-lookup"><span data-stu-id="eb996-130">Before starting the DHCPv6 Server, the host application creates a Server DUID in line 498 and sets the local network DNS server on line 483.</span></span> <span data-ttu-id="eb996-131">次に、行 521 で、割り当て可能な IP アドレスのテーブルが作成されます。</span><span class="sxs-lookup"><span data-stu-id="eb996-131">It then creates a table of assignable IP addresses in lines 521.</span></span> <span data-ttu-id="eb996-132">サーバー テーブルをメモリに格納および取得する方法については、付録 D の「**高度なシステム例**」を参照してください。</span><span class="sxs-lookup"><span data-stu-id="eb996-132">See the **Advanced Example System** in Appendix D for how to store and retrieve Server tables from memory.</span></span>

<span data-ttu-id="eb996-133">その後、行 530 で、DHCPv6 サーバーを起動する準備が整います。</span><span class="sxs-lookup"><span data-stu-id="eb996-133">Then the DHCPv6 Server is ready to start on line 530.</span></span>

<span data-ttu-id="eb996-134">NetX Duo DHCPv6 クライアントの作成と実行の詳細については、DHCPv6 サーバーと共に配布される *nxd_dhcpv6_client.pdf* ファイルを参照してください。</span><span class="sxs-lookup"><span data-stu-id="eb996-134">For details on creating and running the NetX Duo DHCPv6 Client see the *nxd_dhcpv6_client.pdf* file distributed on with the DHCPv6 Server.</span></span>
...
<span data-ttu-id="eb996-135">**図 6.NetX Duo DHCPv6 サーバーの例**</span><span class="sxs-lookup"><span data-stu-id="eb996-135">**Figure 6. Example of the NetX Duo DHCPv6 Server**</span></span>