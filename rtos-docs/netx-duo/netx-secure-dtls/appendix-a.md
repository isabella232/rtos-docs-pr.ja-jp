...
以下の表 2 は、NetX Secure X.509 サービスによって返される可能性があるエラー コードを示しています。 他のエラー コードがサービスによって返される可能性があることにも注意してください。 X.509 戻り値は 0x181 から、TLS 値は 0x101 から始まります。また、TCP/IP 値は 0x100 未満です。 TCP/IP の戻り値に関する情報については、NetX TCP/IP のドキュメントを参照してください。TLS の戻り値については、上記を参照してください。
