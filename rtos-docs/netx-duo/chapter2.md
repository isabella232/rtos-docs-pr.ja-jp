...
|NX_ENABLE_SOURCE_ADDRESS_CHECK| 定義すると、受信パケットの送信元アドレスを確認できます。 既定では、このオプションは無効になっています。|
| NX_IPSEC_ENABLE  | 定義すると、NetX Duo ライブラリが IPsec 操作をサポートできます。 この機能には、オプションの NetX Duo IPsec モジュールが必要です。 既定では、この機能は有効になっていません。            |
| NX_LITTLE_ENDIAN | 定義すると、プロトコル ヘッダーが適切なビッグ エンディアン形式であることを確認するために、リトル エンディアン環境上で必要なバイト スワッピングが実行されます。 既定値は、通常、***nx_port.h*** 内で設定されていることに注意してください。|
...
| NX_PHYSICAL_HEADER  | フレームの物理ヘッダーのサイズをバイト単位で指定します。 既定値は 16 で (32 ビット境界にアラインされた一般的な 14 バイト イーサネット フレームに基づく)、***nx_api.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_*_nx_api.h_*_ が追加される前に、たとえば _ *_nx_user. h_*.* 内で値を定義します。 |
...
|NX_ARP_DEFEND_INTERVAL| ARP モジュールが、競合しているアドレスを示す受信 ARP メッセージの応答として、次の防御パケットを送信する間隔を秒単位で定義します。|
...
|NX_ARP_EXPIRATION_RATE| ARP エントリが有効なまま維持される秒数を指定します。 既定値の 0 は、ARP エントリの有効期限またはエージングを無効にし、***nx_api .h** 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
...
|NX_ARP_MAX_QUEUE_DEPTH | ARP 応答を待機中にキューに入れることができるパケットの最大数を指定します。 既定値は 4 で、***nx_api.h*** 内で定義されています。|
|NX_ARP_MAXIMUM_RETRIES | ARP 応答なしで実行される ARP 再試行の最大数を指定します。 既定値は 18 で、***nx_api.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_ARP_UPDATE_RATE | ARP 再試行の間隔を秒数で指定します。 既定値は 10 で、10 秒を表し、***nx_api.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_DISABLE_ARP_AUTO_ENTRY | 定義すると、ARP キャッシュ内への ARP 要求情報の入力が無効になります。|
|NX_DISABLE_ARP_INFO | 定義すると、ARP 情報の収集が無効になります。|
|NX_ENABLE_ARP_MAC_CHANGE_NOTIFICATION| 定義すると、ARP が MAC アドレス更新を検出したときに、コールバック通知関数を呼び出すことができます。|

### <a name="icmp-configuration-options"></a>ICMP 構成オプション

| オプション  | 説明  |
|---|---|
|NX_DISABLE_ICMP_INFO| 定義すると、ICMP 情報の収集が無効になります。|
...
| オプション  | 説明  |
|---|---|
|NX_DISABLE_IGMP_INFO | 定義すると、IGMP 情報の収集が無効になります。|
|NX_DISABLE_IGMPV2 | 定義すると、IGMPv2 サポートが無効になり、NetX Duo は IGMPv1 のみをサポートするようになります。 既定では、このオプションは設定されておらず、***nx_api.h*** 内で定義されています。|
|NX_MAX_MULTICAST_GROUPS | 参加できるマルチキャスト グループの最大数を指定します。 既定値は 7 で、***nx_api.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|

### <a name="ip-configuration-options"></a>IP 構成オプション
...
| NX_DISABLE_IP_INFO | 定義すると、IP 情報の収集が無効になります。|
...
| オプション  | 説明  |
|---|---|
|NX_DISABLE_RARP_INFO | 定義すると、RARP 情報の収集が無効になります。|

### <a name="tcp-configuration-options"></a>TCP 構成オプション

| オプション | 説明 |
|---|---|
|NX_DISABLE_RESET_DISCONNECT | 定義すると、提供されたタイムアウト値が ***NX_NO_WAIT*** として指定されている場合に、切断時のリセット処理が無効になります。|
|NX_DISABLE_TCP_INFO | 定義すると、TCP 情報の収集が無効になります。|
...
|NX_ENABLE_TCP_KEEPALIVE | 定義すると、オプションの TCP keepalive タイマーが有効になります。 既定の設定は有効になっていません。|
|NX_ENABLE_TCP_MSS_CHECK | 定義すると、TCP 接続を受け入れる前に、最小ピア MSS の検証が有効になります。 この機能を使用するには、シンボル ***NX_ENABLE_TCP_MSS_MINIMUM*** を定義する必要があります。 既定では、このオプションはオフです。|
...
|NX_MAX_LISTEN_REQUESTS | サーバー リッスン要求の最大数を指定します。 既定値は 10 で、***nx_api.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
...
|NX_TCP_ACK_TIMER_RATE | TCP 遅延 ACK 処理のタイマー レート計算のためのシステム ティック数 (NX_IP_PERIODIC_RATE) の分割方法を指定します。 既定値は 5 で、200 ミリ秒を表し、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
...
|NX_TCP_FAST_TIMER_RATE | 高速 TCP タイマー レート計算のための NetX Duo 内部ティック数 (NX_IP_PERIODIC_RATE) の分割方法を指定します。 高速 TCP タイマーは、遅延 ACK タイマーなど、さまざまな TCP タイマーを駆動するために使用されます。 既定値は 10 で、ThreadX タイマーが 10 ミリ秒で実行されていることを前提として、100 ミリ秒を表します。 この値は、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_TCP_IMMEDIATE_ACK| 定義すると、オプションの TCP 即時 ACK 応答処理が有効になります。 このシンボルを定義することは、***NX_TCP_ACK_EVERY_N_PACKETS*** を 1 に定義することと同じです。|
|NX_TCP_KEEPALIVE_INITIAL | keepalive タイマーがアクティブになるまでの非アクティブな時間を秒数で指定します。 既定値は 7200 で、2 時間を表し、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_TCP_KEEPALIVE_RETRIES | 接続が切断されたと見なされるまでの keepalive 再試行許容回数を指定します。 既定値は 10 で、10 回の再試行を表し、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_TCP_KEEPALIVE_RETRY | 接続の一方の側が応答していないことを前提として、keepalive タイマーの再試行の間隔を秒数で指定します。 既定値は 75 で、再試行間隔が 75 秒であることを表し、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_TCP_MAX_OUT_OF_ORDER_PACKETS | TCP ソケット受信キューに保持できる順不同 TCP パケットの最大数を定義するシンボル。 このシンボルを使用して、TCP 受信ソケット内のキューに入っているパケット数を制限し、パケット プールが枯渇するのを防ぐことができます。 既定では、このシンボルは定義されていません。このため、TCP ソケット内のキューに入っている順不同のパケット数に制限はありません。|
|NX_TCP_MAXIMUM_RETRIES | 接続が切断されたと見なされるまでのデータ送信再試行許容回数を指定します。 既定値は 10 で、10 回の再試行を表し、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
|NX_TCP_MAXIMUM_RX_QUEUE | TCP ソケットの最大受信キューを定義するシンボル。 この機能は ***NX_ENABLE_LOW_WATERMARK*** によって有効になります。|
|NX_TCP_MAXIMUM_TX_QUEUE | TCP 送信要求が中断または拒否される前の TCP 送信キューの最大深度を指定します。 既定値は 20 です。これは、特定の時点で最大 20 個のパケットを送信キュー内で保持できることを意味します。 パケット データの一部またはすべてに対応する ACK が接続の一方の側から受信されるまで、パケットが送信キューに残ることに注意してください。 この定数は、***nx_tcp. h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
...
|NX_TCP_RETRY_SHIFT | 再試行間で再送信タイムアウトの長さがどのように変わるかを指定します。 この値が 0 の場合、最初の再送信タイムアウトは、後続の再送信タイムアウトと同じになります。 この値が 1 の場合、連続する再送信はそれぞれ 2 倍の長さになります。 この値が 2 の場合、後続の再送信タイムアウトはそれぞれ 4 倍の長さになります。 既定値は 0 で、***nx_tcp.h** _ 内で定義されています。 アプリケーションで既定値をオーバーライドするには、_ *_nx_api.h_** が追加される前に値を定義します。|
...
| オプション  | 説明  |
|---|---|
|NX_DISABLE_UDP_INFO | 定義すると、UDP 情報の収集が無効になります。|
