...
このサービスでは、現在 ARP キャッシュにあるすべての動的 ARP エントリを無効にします。
...
### <a name="description"></a>説明    
このサービスでは、ARP キャッシュから動的エントリを割り当て、指定された IP を物理アドレス マッピングに設定します。 物理アドレスがゼロに指定されている場合は、その物理アドレスが解決されるために、実際の ARP 要求がネットワークに送信されます。 別の注意すべき点として、ARP エージングがアクティブである場合や、ARP キャッシュが使い果たされていて、これが最も長く使われていない ARP エントリである場合、このエントリは削除されます。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **arp_cache_memory** ARP キャッシュを配置するメモリ領域へのポインター。
- **arp_cache_size** 各 ARP エントリは 52 バイトであるため、ARP エントリの合計数は 52 で割り切れるサイズになります。
...
このサービスでは、インターフェイスの IP アドレスが有効である限り、すべての物理インターフェイスを経由して、Gratuitous ARP 要求を送信します。 その後、ARP 応答を受信した場合は、指定された応答ハンドラーが呼び出されて、Gratuitous ARP への応答を処理します。

### <a name="parameters"></a>パラメーター

- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **response_handler** 応答処理関数へのポインター。 NX_NULL が指定されている場合、応答は無視されます。

### <a name="return-values"></a>戻り値

- **NX_SUCCESS** (0x00) Gratuitous ARP の送信に成功しました。
...
IP アドレスに関連付けられている物理ハードウェア アドレスを検索します
...
このサービスでは、指定された IP アドレスに関連付けられている ARP キャッシュ内の物理ハードウェア アドレスの検索を試みます。

### <a name="parameters"></a>パラメーター 

- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **ip_address** 検索する IP アドレス。
- **physical_msw** その物理アドレスの上位 16 ビット (47-32) を返すための変数へのポインター。
- **physical_lsw** その物理アドレスの下位 32 ビット (31-0) を返すための変数へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **arp_requests_sent** この IP インスタンスから送信された ARP 要求の合計の宛先へのポインター。
- **arp_requests_received** ネットワークから受信した ARP 要求の合計の宛先へのポインター。
- **arp_responses_sent** この IP インスタンスから送信された ARP 応答の合計の宛先へのポインター。
- **arp_responses_received** ネットワークから受信した ARP 応答の合計の宛先へのポインター。
- **arp_dynamic_entries** 現在の動的 ARP エントリ数の宛先へのポインター。
- **arp_static_entries** 現在の静的 ARP エントリ数の宛先へのポインター。
- **arp_aged_entries** 期限切れで無効になった ARP エントリの合計数の宛先へのポインター。
- **arp_invalid_messages** 受信した無効な ARP メッセージの合計の宛先へのポインター。
...
物理アドレスに関連付けられている IP アドレスを検索します
...
このサービスでは、指定された物理アドレスに関連付けられている ARP キャッシュ内の IP アドレスの検索を試みます。
...
このサービスでは、ARP キャッシュ内のすべての静的エントリを削除します。
...
このサービスでは、指定された IP インスタンスの ARP キャッシュに静的 IP から物理アドレスへのマッピングを作成します。 静的 ARP エントリは、ARP の定期的な更新の対象にはなりません。
...
ARP キャッシュにある、静的 IP からハードウェアへのマッピングを削除します
...
このサービスでは、指定された IP インスタンスの ARP キャッシュの中で、静的 IP から物理アドレスへ結び付ける、以前に作成されたマッピングを検索して削除します。
...
このサービスでは、指定された IP インスタンスの ICMP コンポーネントを有効にします。 この ICMP コンポーネントは、インターネット エラー メッセージと ping の要求および応答を処理する役割を果たします。 
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **pings_sent** 送信された ping の合計数の宛先へのポインター。
- **ping_timeouts** ping タイムアウトの合計数の宛先へのポインター。
...
このサービスでは、指定された IP アドレスに ping 要求を送信し、ping 応答メッセージを指定された時間だけ待機します。 応答が受信されない場合は、エラーが返されます。 それ以外の場合は、response_ptr によってポイントされている変数で応答メッセージ全体が返されます。 
...
- **NX_SUCCESS** (0x00) ping に成功しました。 応答メッセージ ポインターは、response_ptr によってポイントされている変数に配置されました。
...
このサービスでは、指定された IP インスタンスの IGMP コンポーネントを有効にします。 IGMP コンポーネントは、IP マルチキャスト グループ管理操作のサポートを提供する役割を果たします。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **igmp_reports_sent** 送信された ICMP レポートの合計数の宛先へのポインター。
- **igmp_queries_received** マルチキャスト ルーターによって受信されたクエリの合計数の宛先へのポインター。
- **igmp_checksum_errors** 受信パケットの IGMP チェックサム エラーの合計数の宛先へのポインター。
- **current_groups_joined** この IP インスタンスを介して参加している現在のグループ数の宛先へのポインター。
...
このサービスにより、それ以降のすべての参加マルチキャスト グループの IGMP ループバックを無効にします。
...
このサービスでは、それ以降のすべての参加マルチキャスト グループの IGMP ループバックを有効にします。
...
このサービスでは、指定されたネットワーク インターフェイス経由で、指定されたマルチキャスト グループに IP インスタンスを参加させます。 同じグループへの参加が何回あったかを追跡するため、内部カウンターが保持されます。 マルチキャスト グループに参加した後、この IGMP コンポーネントにより、このグループ アドレスを持つ IP パケットを、指定されたネットワーク インターフェイス経由で受信することが許可されます。また、この IP がこのマルチキャスト グループのメンバーであることもルーターに報告されます。 IGMP メンバーシップの参加、報告、脱退のメッセージも、指定されたネットワーク インターフェイス経由で送信されます。 IGMP グループ メンバーシップ レポートを送信せずに IPv4 マルチキャスト グループに参加するには、アプリケーションでサービス ***nx_ipv4_multicast_interface_join*** を使用する必要があります。
...
このサービスでは、指定されたマルチキャスト グループに IP インスタンスを参加させます。 同じグループへの参加が何回あったかを追跡するため、内部カウンターが保持されます。 これが、ホストがグループに参加しようとしていることを示す、ネットワーク上に送信される最初の参加要求である場合、ドライバーは IGMP レポートを送信するように命令されます。 参加後に、この IGMP コンポーネントは、このグループ アドレスを使用した IP パケットの受信を許可し、その IP がこのマルチキャスト グループのメンバーであることをルーターに報告します。 IGMP グループ メンバーシップ レポートを送信せずに IPv4 マルチキャスト グループに参加するには、アプリケーションでサービス ***nx_ipv4_multicast_interface_join*** を使用する必要があります。
...
指定されたマルチキャスト グループから IP インスタンスを脱退させます
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **change_notify** IP 変更通知関数へのポインター。 このパラメーターが NX_NULL の場合、IP アドレスの変更通知は無効になります。
- **additional_info** IP アドレスが変更されたときに通知関数にも提供されるオプションの追加情報へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **ip_address** IP アドレスの宛先へのポインター。
- **network_mask** ネットワーク マスクの宛先へのポインター。
...
このサービスでは、ユーザーが指定した IP アドレスとネットワーク ドライバーを使用して、IP インスタンスを作成します。 さらに、アプリケーションでは、IP インスタンスが内部パケット割り当てに使用できるよう、以前に作成されたパケット プールを提供する必要があります。 この IP のスレッドが実行されるまで、指定されたアプリケーション ネットワーク ドライバーは呼び出されないことにご注意ください。

### <a name="parameters"></a>パラメーター

- **ip_ptr** 新しい IP インスタンスを作成するための制御ブロックへのポインター。
...
- **memory_ptr** IP ヘルパー スレッドのスタック領域のメモリ領域へのポインター。
...
このサービスでは、以前に作成した IP インスタンスを削除し、その IP インスタンスによって所有されていたすべてのシステム リソースを解放します。
...
- **NX_SOCKETS_BOUND** (0x28) この IP インスタンスには依然として UDP または TCP ソケットがバインドされています。 IP インスタンスを削除する前に、すべてのソケットをバインド解除し、削除する必要があります。
- **NX_PTR_ERROR** (0x07) IP ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
...
このサービスでは、***nx_ip_create*** の呼び出し時に指定されたアプリケーションのプライマリ ネットワーク インターフェイス ドライバーに対する直接インターフェイスを提供します。 アプリケーション固有のコマンドは、その数値が NX_LINK_USER_COMMAND 以上である場合に使用できます。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **command** 数値コマンド コード。 標準コマンドは次のように定義されます。
...
- **return_value_ptr** 呼び出し元の戻り変数へのポインター。
...
このサービスでは、IP インスタンス内のアプリケーションのネットワーク デバイス ドライバーに対する直接コマンドを提供します。 アプリケーション固有のコマンドは、その数値が *NX_LINK_USER_COMMAND* 以上である場合に使用できます。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **ip_total_packets_sent** 送信された IP パケットの合計数の宛先へのポインター。
- **ip_total_bytes_sent** 送信された合計バイト数の宛先へのポインター。
- **ip_total_packets_received** IP 受信パケットの合計数の宛先へのポインター。
- **ip_total_bytes_received** 受信した IP バイトの合計数の宛先へのポインター。
- **ip_invalid_packets** 無効な IP パケットの合計数の宛先へのポインター。
- **ip_receive_packets_dropped** 破棄された受信パケットの合計数の宛先へのポインター。
- **ip_receive_checksum_errors** 受信パケットのチェックサム エラーの合計数の宛先へのポインター。
- **ip_send_packets_dropped** 破棄された送信パケットの合計数の宛先へのポインター。
- **ip_total_fragments_sent** 送信されたフラグメントの合計数の宛先へのポインター。
- **ip_total_fragments_received** 受信したフラグメントの合計数の宛先へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **interface_index** インターフェイスのインデックス。IP インスタンスに接続されているネットワーク インターフェイスのインデックスと同じ値です。
- **ip_address** デバイス インターフェイスの IP アドレスの宛先へのポインター。
- **network_mask** デバイス インターフェイスのネットワーク マスクの宛先へのポインター。
...
このサービスでは、物理ネットワーク インターフェイスを IP インターフェイスに追加します。 IP インスタンスはプライマリ インターフェイスを使用して作成されるので、追加の各インターフェイスはプライマリ インターフェイスのセカンダリになります。 この IP インスタンスに接続されているネットワーク インターフェイスの総数 (プライマリ インターフェイスを含む) は、**NX_MAX_PHYSICAL_INTERFACES** を超えることはできません。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **interface_name** インターフェイス名の文字列へのポインター。
...
このサービスでは、指定されたネットワーク インターフェイスのネットワーク パラメーターに関する情報を取得します。 すべてのデータは、ホストのバイト順に取得されます。 
...
- **interface_name** ネットワーク インターフェイスの名前を保持するバッファーへのポインター。
- **ip_address** インターフェイスの IP アドレスの宛先へのポインター。
- **network_mask** ネットワーク マスクの宛先へのポインター。
- **mtu_size** このインターフェイスの最大転送単位の宛先へのポインター。
- **physical_address_msw** デバイスの MAC アドレスの上位 16 ビットの宛先へのポインター。
- **physical_address_lsw** デバイスの MAC アドレスの下位 32 ビットの宛先へのポインター。
...
このサービスでは、以前に作成された IP インスタンスのネットワーク インターフェイスの指定された状態を確認したり、必要に応じてそれを待機したりします。
...
- **NX_PTR_ERROR** (0x07) IP ポインターが無効であるかそうなった、あるいは実際の状態ポインターが無効です。
...
このサービスは、リンク状態の変更通知コールバック関数を構成します。 ユーザーが指定した *link_status_change_notify* ルーチンは、プライマリかセカンダリのいずれかのインターフェイスの状態が変更されたとき (IP アドレスの変更など) に呼び出されます。*link_status_change_notify* が null 値の場合、リンク状態の変更通知コールバック機能は無効になります。
...
- **NX_SUCCESS** (0x00) 設定に成功しました。
- **NX_PTR_ERROR** (0x07) IP 制御ブロックのポインターまたは新しい物理アドレスのポインターが無効です。
- **NX_CALLER_ERROR** (0x11) サービスが、システムの初期化またはスレッド コンテキストから呼び出されていません。
...
このサービスでは、この IP インスタンスの生 IP パケットの送信と受信を無効にします。 生パケットのサービスが以前に有効になっていて、生パケットが受信キューに存在する場合、受信済みの生パケットはすべてこのサービスによって解放されます。
...
このサービスでは、指定された IP インスタンスから生 IP パケットを受信します。 生パケット受信キューに IP パケットがある場合、最初の (最も古い) パケットが呼び出し元に返されます。 それ以外の場合、使用可能なパケットがない場合は、待機オプションの指定に従って呼び出し元が中断される場合があります。
...
- **NX_WAIT_ABORTED** (0x1A) 要求された中断が tx_thread_wait_abort への呼び出しによって中止されました。
- **NX_NOT_ENABLED** (0x14) このコンポーネントは有効になっていません。
- **NX_PTR_ERROR** (0x07) IP またはリターン パケット ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **packet_ptr** 送信する生 IP パケットへのポインター。
...
このサービスでは、指定されたローカル IPv4 アドレスを発信元アドレスとして使用し、関連付けられているネットワーク インターフェイスを経由して、宛先 IP アドレスに生 IP パケットを送信します。 このルーチンはすぐに戻るため、IP パケットが実際に送信されたかどうかはわからないことにご注意ください。 送信が完了したときに、ネットワーク ドライバーがパケットを解放する役割を果たします。 このサービスは、パケットが実際に送信されたかどうかを知る方法がないという点で、他のサービスとは異なります。 パケットはインターネット上で失われているおそれがあります。
...
- **NX_OVERFLOW** (0x03) パケットの先頭追加ポインターが無効です。
- **NX_UNDERFLOW** (0x02) パケットの先頭追加ポインターが無効です。
- **NX_INVALID_INTERFACE** (0x4C) 指定されたインターフェイスのインデックスが無効です。
...
このサービスでは、静的ルーティング テーブルにエントリを追加します。 *next_hop* アドレスは、いずれかのローカル ネットワーク デバイスから直接アクセスできる必要があることにご注意ください。
...
このサービスでは、以前に作成された IP インスタンスのプライマリ ネットワーク インターフェイスの指定された状態を確認したり、必要に応じてそれを待機したりします。 セカンダリ インターフェイスの状態を取得するには、アプリケーションでサービス ***nx_ip_interface_status_check*** を使用する必要があります。
...
このサービスでは、指定されたプールからパケットを割り当て、指定されたパケットの種類に従ってパケット内の先頭追加ポインターを調整します。 使用可能なパケットがない場合、サービスは指定された待機オプションに従って中断されます。
...
- **wait_option** パケット プールに使用可能なパケットがない場合の待機時間をティック数で定義します。 この待機オプションは次のように定義されます。
  - **NX_NO_WAIT** (0x00000000)
  - **NX_WAIT_FOREVER** (0xFFFFFFFF)
  - **タイムアウト値 (ティック数)** (0x00000001 から 0xFFFFFFFE)
...
- **NX_NO_PACKET** (0x01) 使用可能なパケットがありません。
- **NX_WAIT_ABORTED** (0x1A) 要求された中断が tx_thread_wait_abort への呼び出しによって中止されました。
- **NX_INVALID_PARAMETERS** (0x4D) パケット サイズがプロトコルをサポートできません。
- **NX_OPTION_ERROR** (0x0A) パケットの種類が無効です。
- **NX_PTR_ERROR** (0x07) プールまたはパケットの戻りポインターが無効です。
- **NX_CALLER_ERROR** (0x11) スレッド以外からの待機オプションは無効です。
...
このサービスでは、指定されたパケットの情報を、指定されたパケット プールから割り当てられている 1 つ以上の新しいパケットにコピーします。 成功した場合、新しいパケットへのポインターが、**new_packet_ptr** によってポイントされている宛先に返されます。

### <a name="parameters"></a>パラメーター

- **packet_ptr** 発信元パケットへのポインター。
- **new_packet_ptr** パケットの新しいコピーへのポインターを返す元となる宛先へのポインター。
- **pool_ptr** そのコピーのために 1 つ以上のパケットを割り当てるために使用される、以前に作成されたパケット プールへのポインター。
...
このサービスでは、指定されたパケットの末尾にデータを追加します。 指定されたデータ領域がパケットにコピーされます。 使用可能なメモリが不足していて、チェーン化されたパケット機能が有効になっている場合、要求を満たすために 1 つ以上のパケットが割り当てられます。 チェーン化されたパケット機能が有効になっていない場合は、*NX_SIZE_ERROR* が返されます。

### <a name="parameters"></a>パラメーター

- **packet_ptr** パケット ポインター。
- **data_start** パケットに追加するユーザーのデータ領域の先頭へのポインター。
- **data_size** ユーザーのデータ領域のサイズ。
- **pool_ptr** 現在のパケットに十分な空き領域がない場合に、別のパケットの割り当て元となるパケット プールへのポインター。
...
- **NX_WAIT_ABORTED** (0x1A) 要求された中断が tx_thread_wait_abort への呼び出しによって中止されました。
- **NX_INVALID_PARAMETERS** (0x4D) パケット サイズがプロトコルをサポートできません。
- **NX_UNDERFLOW** (0x02) 先頭追加ポインターがペイロードの先頭未満です。
...
このサービスでは、NetX Duo パケット (またはパケット チェーン) のデータを、パケット先頭追加ポインターからのオフセットで指定した起点から、バイト単位で指定したサイズだけ、指定されたバッファーにコピーします。 実際にコピーされたバイト数が *bytes_copied* に返されます。 このサービスによって、パケットからデータが削除されることはなく、先頭追加ポインターやその他の内部状態情報が調整されることもありません。

### <a name="parameters"></a>パラメーター

- **packet_ptr** 抽出するパケットへのポインター。
- **offset** 現在の先頭追加ポインターからのオフセット。
- **buffer_start** 保存バッファーの先頭へのポインター。
...
このサービスは、指定されたパケットから指定されたバッファーにデータをコピーします。 コピーされた実際のバイト数は、**bytes_copied** によってポイントされている宛先に返されます。
...
- **packet_ptr** 発信元パケットへのポインター。
- **buffer_start** バッファー領域の先頭へのポインター。
- **bytes_copied** コピーされたバイト数の宛先へのポインター。
...
このサービスでは、指定されたパケット内のデータの長さを取得します。
...
このサービスでは、ユーザーが指定したメモリ領域に、指定されたパケット サイズのパケット プールを作成します。

### <a name="parameters"></a>パラメーター

- **pool_ptr** パケット プール制御ブロックへのポインター。
- **name** パケット プールのアプリケーションの名前へのポインター。
- **payload_size** プール内の各パケットのバイト数。 この値は、40 バイト以上である必要があり、4 で割り切れなければなりません。
- **memory_ptr** パケット プールを配置するメモリ領域へのポインター。 このポインターは、ULONG 境界上に配置する必要があります。
...
- **pool_ptr** 以前に作成されたパケット プールへのポインター。
- **total_packets** プール内のパケットの合計数の宛先へのポインター。
- **free_packets** 現在解放されているパケットの合計数の宛先へのポインター。
- **empty_pool_requests** プールが空だった場合の割り当て要求の合計数の宛先へのポインター。
- **empty_pool_suspensions** 空のプールによる中断の合計数の宛先へのポインター。
- **invalid_packet_releases** 無効なパケット解放の合計数の宛先へのポインター。
...
このサービスでは、パケットを解放します。これには、指定されたパケットにチェーンされている追加のパケットも含まれます。 パケット割り当てで別のスレッドがブロックされている場合は、それにこのパケットが与えられて、それが再開されます。
...
TCP 以外のパケットの場合、このサービスによって送信済みパケットが解放されます。これには、指定されたパケットにチェーンされている追加のパケットも含まれます。 パケット割り当てで別のスレッドがブロックされている場合は、それにこのパケットが与えられて、それが再開されます。 送信済みの TCP パケットの場合、パケットは送信中としてマークされますが、パケットの受信が確認されるまでは解放されません。 通常、このサービスは、パケットが送信された後にアプリケーションのネットワーク ドライバーから呼び出されます。
...
このサービスを使用すると、特定の IP インスタンスの NetX Duo の RARP コンポーネントが有効になります。 RARP コンポーネントでは、接続されているすべてのネットワーク インターフェイスから IP アドレスがゼロであるものを検索します。 IP アドレスがゼロであることは、そのインターフェイスに IP アドレスがまだ割り当てられていないことを示します。 RARP では、そのインターフェイスで RARP プロセスを有効にすることにより、IP アドレスの解決が試みられます。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **rarp_requests_sent** 送信された RARP 要求の合計数の宛先へのポインター。
- **rarp_responses_received** 受信した RARP 応答の合計数の宛先へのポインター。
- **rarp_invalid_messages** 無効なメッセージの合計数の宛先へのポインター。
...
このサービスでは、以前に作成された TCP クライアント ソケットを、指定された TCP ポートにバインドします。 有効な TCP ソケットの範囲は 0 から 0xFFFF です。 指定された TCP ポートが使用できない場合は、サービスは指定された待機オプションに従って中断されます。
...
このサービスでは、以前に作成され、バインドされた TCP クライアント ソケットを、指定されたサーバーのポートに接続します。 有効な TCP サーバー ポートの範囲は 0 から 0xFFFF です。 接続がすぐに完了しない場合、サービスは指定された待機オプションに従って中断されます。
...
- **socket_ptr** 以前に作成された TCP ソケット インスタンスへのポインター。
- **port_ptr** 戻りポート番号の宛先へのポインター。 有効なポート番号は、1 から 0xFFFF です。
...
このサービスでは、TCP クライアント ソケットと TCP ポートとの間のバインドを解放します。 同じポート番号に別のソケットをバインドするために待機している他のスレッドがある場合は、中断されている最初のスレッドがこのポートにバインドされます。
...
このサービスでは、アプリケーションで指定されているポートを起点として、空いている TCP ポート (バインドされていないもの) を見つけようとします。 この検索ロジックは、検索が最大ポート値 0xFFFF に到達すると、ラップアラウンドします。 検索が成功すると、*free_port_ptr* によってポイントされている変数に空きポートが返されます。
...
- **free_port_ptr** 宛先の空きポートの戻り値へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **tcp_packets_sent** 送信された TCP パケットの合計数の宛先へのポインター。
- **tcp_bytes_sent** 送信された TCP バイトの合計数の宛先へのポインター。
- **tcp_packets_received** 受信した TCP パケットの合計数の宛先へのポインター。
- **tcp_bytes_received** 受信した TCP バイトの合計数の宛先へのポインター。
- **tcp_invalid_packets** 無効な TCP パケットの合計数の宛先へのポインター。
- **tcp_receive_packets_dropped** 破棄された TCP 受信パケットの合計数の宛先へのポインター。
- **tcp_checksum_errors** チェックサム エラーが発生している TCP パケットの合計数の宛先へのポインター。
- **tcp_connections** TCP 接続の合計数の宛先へのポインター。
- **tcp_disconnections** TCP 切断の合計数の宛先へのポインター。
- **tcp_connections_dropped** 破棄された TCP 接続の合計数の宛先へのポインター。
- **tcp_retransmit_packets** 再送信された TCP パケットの合計数の宛先へのポインター。
...
- **socket_ptr** TCP サーバー ソケット制御ブロックへのポインター。
- **wait_option** 接続を確立している最中のサービスの動作を定義します。 この待機オプションは次のように定義されます。
    - **NX_NO_WAIT** (0x00000000)
    - **NX_WAIT_FOREVER** (0xFFFFFFFF)
    - **タイムアウト値 (ティック数)** (0x00000001 から 0xFFFFFFFE)
...
このサービスでは、指定された TCP ポートでのクライアント接続要求のリッスンを有効にします。 クライアント接続要求を受信すると、指定されたサーバー ソケットが指定されたポートにバインドされ、指定されたリッスン コールバック関数が呼び出されます。

リッスン コールバック ルーチンの処理は、全面的にアプリケーションによって決まります。 それに含まれているロジックにより、後から受け入れ操作を実行するアプリケーション スレッドがウェイクアップされる場合もあります。 このソケットの受け入れ処理で中断しているスレッドが既にアプリケーションにある場合、リッスン コールバック ルーチンは必要ではない可能性があります。

そのアプリケーションで同じポート上の追加のクライアント接続を処理する場合は、次の接続のために使用可能なソケット (CLOSED 状態のソケット) を使用して ***nx_tcp_server_socket_relisten*** を呼び出す必要があります。 再リッスン サービスが呼び出されるまで、追加のクライアント接続はキューに入れられます。 キューの最大深度を超えた場合は、新しい接続要求をキューできるようにするため、最も古い接続要求が破棄されます。 キューの最大深度は、このサービスによって指定されます。
...
- **NX_CONNECTION_PENDING** (0x48) NX_SUCCESS と同じですが、キューに入れられた接続要求が既に存在しており、この呼び出し中にその要求が処理されたという点が異なります。
- **NX_INVALID_PORT** (0x46) 指定されたポートが無効です。
...
このサービスでは、このサーバー ソケットと指定されたサーバー ポートとの間の関連付けを削除します。 切断の後、または受け入れ呼び出しが失敗した後に、アプリケーションからこのサービスを呼び出す必要があります。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に設定されたサーバーのソケット インスタンスへのポインター。

### <a name="return-values"></a>戻り値  

- **NX_SUCCESS** (0x00) サーバー ソケットの受け入れ解除に成功しました。
- **NX_NOT_LISTEN_STATE** (0x36) サーバー ソケットが不適切な状態のため、切断されていないおそれがあります。
- **NX_PTR_ERROR** (0x07) ソケット ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
- **NX_NOT_ENABLED** (0x14) このコンポーネントは有効になっていません。
...
このサービスでは、指定された TCP ポートでのクライアント接続要求のリッスンを無効にします。
...
このサービスでは、指定された TCP ソケットで取得可能なバイト数を取得します。 TCP ソケットが既に接続されている必要があることにご注意ください。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に作成され、接続されている TCP ソケットへのポインター。
- **bytes_available** 使用可能なバイトの宛先へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **socket_ptr** 新しい TCP ソケット制御ブロックへのポインター。
...
- **time_to_live** このパケットが破棄されるまでに通過できるルーターの数を定義する 8 ビット値を指定します。 既定値は NX_IP_TIME_TO_LIVE によって指定されます。
- **window_size** このソケットの受信キューで許可される最大バイト数を定義します。
- **urgent_data_callback** 受信ストリームで緊急データが検出されるたびに呼び出されるアプリケーション関数。 この値が NX_NULL の場合、緊急データは無視されます。
- **disconnect_callback** 接続のもう一方の終端でソケットによる切断が発行されるたびに呼び出されるアプリケーション関数。 この値が NX_NULL の場合、切断コールバック関数は無効になります。
...
このサービスでは、以前に作成された TCP ソケットを削除します。 そのソケットがまだバインドされているか接続されている場合、サービスからエラー コードが返されます。
...
このサービスでは、確立されたクライアントまたはサーバーのソケット接続を切断します。 サーバー ソケットを切断した場合、その後に受け入れ解除要求が発行される必要がありますが、切断されているクライアント ソケットは別の接続要求を受け入れられる状態のままになります。 切断プロセスをすぐに完了できない場合、サービスは指定された待機オプションに従って中断されます。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に接続されたクライアントまたはサーバー ソケット インスタンスへのポインター。
...
- **socket_ptr** 以前に作成された TCP ソケット インスタンスへのポインター。
- **tcp_packets_sent** ソケットで送信された TCP パケットの合計数の宛先へのポインター。
- **tcp_bytes_sent** ソケットで送信された TCP バイトの合計数の宛先へのポインター。
- **tcp_packets_received** ソケットで受信した TCP パケットの合計数の宛先へのポインター。
- **tcp_bytes_received** ソケットで受信した TCP バイトの合計数の宛先へのポインター。
- **tcp_retransmit_packets** TCP パケット再送信の合計数の宛先へのポインター。
- **tcp_packets_queued** ソケットでキューに入れられた TCP パケットの合計数の宛先へのポインター。
- **tcp_checksum_errors** ソケットでチェックサム エラーが発生している TCP パケットの合計数の宛先へのポインター。
- **tcp_socket_state** ソケットの現在の状態の宛先へのポインター。
- **tcp_transmit_queue_depth** ACK を待機してキューに入れられたままになっている送信パケットの合計数の宛先へのポインター。
- **tcp_transmit_window** 現在の送信ウィンドウ サイズの宛先へのポインター。
- **tcp_receive_window** 現在の受信ウィンドウ サイズの宛先へのポインター。
...
このサービスでは、指定されたソケットのローカルの最大セグメント サイズ (MSS) を取得します。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に作成されたソケットへのポインター。
- **mss** MSS を返す宛先。
...
このサービスでは、ピア ソケットによってアドバタイズされた最大セグメント サイズ (MSS) を取得します。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に作成され、接続されているソケットへのポインター。
- **mss** MSS を返す宛先。
...
このサービスでは、指定されたソケットの最大セグメント サイズ (MSS) を設定します。 MSS 値は、ネットワーク インターフェイスの IP MTU の範囲内で指定して、IP および TCP ヘッダーのための余地を残す必要があることにご注意ください。

TCP ソケットで接続プロセスが開始される前にこのサービスを使用する必要があります。 TCP 接続が確立された後にこのサービスを使用した場合、新しい値は接続に影響しません。
...
- **peer_ip_address** ピア IP アドレスの宛先へのポインター (ホストのバイト順)。
- **peer_port** ピア ポート番号の宛先へのポインター (ホストのバイト順)。
...
- **NX_SUCCESS** (0x00) ソケット データの受信に成功しました。
- **NX_NOT_BOUND** (0x24) ソケットがまだバインドされていません。
- **NX_NO_PACKET** (0x01) 受信したデータはありません。
- **NX_WAIT_ABORTED** (0x1A) 要求された中断が tx_thread_wait_abort への呼び出しによって中止されました。
- **NX_NOT_CONNECTED** (0x38) ソケットはもう接続されていません。
- **NX_PTR_ERROR** (0x07) ソケットまたはリターン パケット ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
- **NX_NOT_ENABLED** (0x14) このコンポーネントは有効になっていません。
...
このサービスでは、アプリケーションによって指定されたコールバック関数を使用して、受信通知関数ポインターを構成します。 そのようにすると、そのソケットで 1 つ以上のパケットを受信するたびに、このコールバック関数が呼び出されます。 NX_NULL ポインターが指定されている場合、この通知関数は無効になります。

### <a name="parameters"></a>パラメーター

- **socket_ptr** TCP ソケットへのポインター。
- **tcp_receive_notify** ソケットで 1 つ以上のパケットを受信したときに呼び出されるアプリケーション コールバック関数ポインター。

### <a name="return-values"></a>戻り値 

- **NX_SUCCESS** (0x00) ソケットの受信通知に成功しました。
- **NX_PTR_ERROR** (0x07) ソケット ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
- **NX_NOT_ENABLED** (0x14) TCP 機能が有効になっていません。
...
このサービスでは、以前に接続された TCP ソケット経由で TCP データを送信します。 受信側の最後にアドバタイズされたウィンドウ サイズがこの要求未満である場合、指定された待機オプションに基づいて、サービスが必要に応じて中断されます。 このサービスにより、MSS を超えるパケット データが IP 層に送信されないことが保証されます。 
...
- **socket_ptr** 以前に接続された TCP ソケット インスタンスへのポインター。
- **packet_ptr** TCP データ パケット ポインター。
- **wait_option** 要求が受信側のウィンドウ サイズを超える場合のサービスの動作を定義します。 この待機オプションは次のように定義されます。
    - **NX_NO_WAIT** (0x00000000)
    - **NX_WAIT_FOREVER** (0xFFFFFFFF)
    - **タイムアウト値 (ティック数)** (0x00000001 から 0xFFFFFFFE)
...
- **NX_NOT_CONNECTED** (0x38) ソケットはもう接続されていません。
- **NX_WINDOW_OVERFLOW** (0x39) 要求が、受信側のアドバタイズされたウィンドウ サイズ (バイト単位) を超えています。
- **NX_WAIT_ABORTED** (0x1A) 要求された中断が tx_thread_wait_abort への呼び出しによって中止されました。
- **NX_INVALID_PACKET** (0x12) パケットが割り当てられていません。
- **NX_TX_QUEUE_DEPTH** (0x49) 転送キューの最大深度に達しました。
- **NX_OVERFLOW** (0x03) パケットの追加ポインターが無効です。
- **NX_PTR_ERROR** (0x07) ソケット ポインターが無効です。
- **NX_CALLER_ERROR** (0x11) このサービスの呼び出し元が無効です。
- **NX_NOT_ENABLED** (0x14) このコンポーネントは有効になっていません。
...
このサービスでは、ソケットが目的の状態になるのを待機します。 ソケットが目的の状態でない場合、サービスは指定された待機オプションに従って中断されます。
...
このサービスでは、指定された TCP ソケットのさまざまな送信パラメーターを構成します。

### <a name="parameters"></a>パラメーター

- **socket_ptr** TCP ソケットへのポインター。
- **max_queue_depth** 送信するためにキューに入れることができるパケットの最大数。
- **timeout** パケットをもう一度送信する前に ACK を待機する ThreadX タイマー ティック数。
- **max_retries** 許可される最大再試行回数。
- **timeout_shift** 再試行のたびにタイムアウトをシフトする値。 値が 0 の場合は、再試行が続いてもタイムアウトに変わりはありません。 値が 1 の場合は、再試行のたびにタイムアウトが 2 倍になります。
...
このサービスを使用すると、ソケット ウィンドウ更新コールバック ルーチンがインストールされます。 指定されたソケットがリモート ホストのウィンドウ サイズが増加したことを示すパケットを受信するたびに、このルーチンが自動的に呼び出されます。
...
このサービスでは、アプリケーションで指定されているポート番号を起点として、空いている UDP ポート (バインドされていないもの) を検索します。 この検索ロジックは、検索が最大ポート値 0xFFFF に到達すると、ラップアラウンドします。 検索が成功すると、free_port_ptr によってポイントされている変数に空きポートが返されます。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **port** 検索を開始するポート番号 (1 から 0xFFFF)。
- **free_port_ptr** 宛先の空きポートの戻り変数へのポインター。
...
- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **udp_packets_sent** 送信された UDP パケットの合計数の宛先へのポインター。
- **udp_bytes_sent** 送信された UDP バイトの合計数の宛先へのポインター。
- **udp_packets_received** 受信した UDP パケットの合計数の宛先へのポインター。
- **udp_bytes_received** 受信した UDP バイトの合計数の宛先へのポインター。
- **udp_invalid_packets** 無効な UDP パケットの合計数の宛先へのポインター。
- **udp_receive_packets_dropped** 破棄された UDP 受信パケットの合計数の宛先へのポインター。
- **udp_checksum_errors** チェックサム エラーが発生している UDP パケットの合計数の宛先へのポインター。
...
- **protocol** プロトコル (UDP) へのポインター。
- **port** 送信元のポート番号へのポインター。
- **interface_index** 受信インターフェイスのインデックスへのポインター。
...
このサービスでは、以前に作成された UDP ソケットを、指定された UDP ポートにバインドします。 有効な UDP ソケットの範囲は 0 から 0xFFFF です。 要求されたポート番号が別のソケットにバインドされている場合、このサービスは、そのポート番号からソケットがバインド解除されるまで、指定された時間待機します。
...
このサービスでは、指定された UDP ソケットでの受信に使用可能なバイト数を取得します。

### <a name="parameters"></a>パラメーター

- **socket_ptr** 以前に作成された UDP ソケットへのポインター。
- **bytes_available** 使用可能なバイトの宛先へのポインター。
...
このサービスでは、指定された UDP ソケットでパケットを送受信するためのチェックサム ロジックを無効にします。 チェックサム ロジックが無効になっている場合、このソケットを介して送信されるすべてのパケットの UDP ヘッダーのチェックサム フィールドに値 0 が読み込まれます。 UDP ヘッダーのゼロ値のチェックサム値は、このパケットのチェックサムが計算されていないことを受信者に示す働きをします。
...
このサービスでは、指定された UDP ソケットでパケットを送受信するためのチェックサム ロジックを有効にします。 このチェックサムでは、UDP データ領域全体と擬似 IP ヘッダーが対象になります。
...
このサービスでは、指定された IP インスタンスの UDP ソケットを作成します。

### <a name="parameters"></a>パラメーター

- **ip_ptr** 以前に作成された IP インスタンスへのポインター。
- **socket_ptr** 新しい UDP ソケット制御ブロックへのポインター。
...
このサービスでは、以前に作成された UDP ソケットを削除します。 ソケットがポートにバインドされている場合は、最初にソケットをバインド解除する必要があります。
...
- **udp_bytes_sent** ソケットで送信された UDP バイトの合計数の宛先へのポインター。
- **udp_packets_received** ソケットで受信した UDP パケットの合計数の宛先へのポインター。
- **udp_bytes_received** ソケットで受信した UDP バイトの合計数の宛先へのポインター。
- **udp_packets_queued** ソケットでキューに入れられた UDP パケットの合計数の宛先へのポインター。
- **udp_receive_packets_dropped** キューのサイズを超えたため、ソケットで破棄された UDP 受信パケットの合計数の宛先へのポインター。
- **udp_checksum_errors** ソケットでチェックサム エラーが発生している UDP パケットの合計数の宛先へのポインター。
...
このサービスでは、指定されたソケットから UDP データグラムを受信します。 指定されたソケットでキューに入れられたデータグラムが存在しない場合は、指定された待機オプションに基づいて呼び出し元が中断されます。
...
- **socket_ptr** 以前に作成された UDP ソケット インスタンスへのポインター。
- **packet_ptr** UDP データグラム パケットのポインターへのポインター。
- **wait_option** このソケットでデータグラムが現在キューに入れられていない場合のサービスの動作を定義します。 この待機オプションは次のように定義されます。
    - **NX_NO_WAIT** (0x00000000)
    - **NX_WAIT_FOREVER** (0xFFFFFFFF)
    - **タイムアウト値 (ティック数)** (0x00000001 から 0xFFFFFFFE)
...
このサービスでは、以前に作成され、バインドされた UDP ソケットを介して、指定された IP アドレスを発信元アドレスとするネットワーク インターフェイス経由で UDP データグラムを送信します。 このサービスは、UDP データグラムが正常に送信されたかどうかに関係なく、直ちに返されることにご注意ください。 ***nxd_udp_socket_source_send*** は、IPv4 および IPv6 の両方のネットワークで機能します。
...
このサービスでは、UDP ソケットと UDP ポートとの間のバインドを解放します。 バインド解除操作の一環として、受信キューに格納されているすべての受信パケットが解放されます。

バインドが解除されるポートに別のソケットをバインドするために待機している他のスレッドがある場合は、中断されている最初のスレッドが新たにバインドが解除されたポートにバインドされます。
...
- **packet_ptr** UDP データグラム パケットのポインター。
- **ip_address** 戻り IP アドレス変数への有効なポインター。
- **port** 戻りポート変数への有効なポインター。

### <a name="return-values"></a>戻り値 

- **NX_SUCCESS** (0x00) 発信元 IP またはポートの抽出に成功しました。
